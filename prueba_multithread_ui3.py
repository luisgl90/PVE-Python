...
from PyQt5.QtWidgets import QMainWindow, QApplication, QDialog, QWidget, QPushButton, QLabel, QLineEdit, QComboBox, QTabWidget, QMessageBox, QAction
...
class UI(QMainWindow):
	def __init__(self):
		...
		self.param_cog = {}
		self.param_cog['L_der'] = ""	# Distancia llantas derechas
		self.param_cog['L_izq'] = ""	# Distancia llantas izquierdas
		self.param_cog['bf'] = ""		# Distancia llantas frontales
		self.param_cog['bt'] = ""		# Distancia llantas traseras
		self.param_cog['dw_der'] = ""	# Ancho llanta delantera derecha sin achatar
		self.param_cog['dw_izq'] = ""	# Ancho llanta delantera izquierda sin achatar
		self.param_cog['d_w_der'] = ""	# Ancho llanta delantera derecha achatada
		self.param_cog['d_w_izq'] = ""	# Ancho llanta delantera derecha achatada
		self.param_cog['hb'] = ""		# Altura de básculas
		self.param_cog['B1_f'] = []		# Masa frontal - báscula 1 en plano
		self.param_cog['B2_f'] = []		# Masa frontal - báscula 2 en plano
		self.param_cog['B1_t'] = []		# Masa trasera - báscula 1 en plano
		self.param_cog['B2_t'] = []		# Masa trasera - báscula 2 en plano
		self.param_cog['B1_th'] = []	# Masa trasera - báscula 1 en elevación
		self.param_cog['B2_th'] = []	# Masa trasera - báscula 2 en elevación
		self.param_cog['H1_f'] = []		# Altura de elevación 1
		self.param_cog['H2_f'] = []		# Altura de elevación 2

		self.cnt_fase_cog = 0
		self.fase_cog = 1
		self.fases_cog = ['Parámetros iniciales','Masa delantera en plano','Masa trasera en plano',
			'Masa trasera en elevación']
		self.desc_cog = ['Para iniciar, ingrese los parámetros del<br> vehículo y presione el botón <b><em>Siguiente></em></b>.',
			'Ubique las básculas en las llantas delanteras <br>y capture 3 medidas de la masa. Luego presione <br>el botón <b><em>Siguiente</em></b>.',
			'Ubique las básculas en las llantas traseras <br>y capture 3 medidas de la masa. Luego presione <br>el botón <b><em>Siguiente</em></b>.',
			'Sin mover las básculas, eleve el vehículo a 3<br>alturas diferentes y capture medidas de la masa. <br>Luego presione el botón <b><em>Finalizar</em></b>.']
	...
	def setupUi(self):
		uic.loadUi("interfaz_app_v2.1.ui",self)
		self.setWindowIcon(QIcon('icons/Univalle.png'))
		...
		self.label_c_tit1 = self.findChild(QLabel,"label_cog_tit_1")
		self.label_c_tit2 = self.findChild(QLabel,"label_cog_tit_2")
		self.label_c_desc = self.findChild(QLabel,"label_cog_desc")
		self.label_c_1 = self.findChild(QLabel,"label_cog_1")
		self.label_c_1.setText("𝑳<sub>der</sub> (mm)")
		self.label_c_1.setToolTip('Distancia entre centros de llantas del lado derecho.')
		self.label_c_2 = self.findChild(QLabel,"label_cog_2")
		self.label_c_2.setText("𝑳<sub>izq</sub> (mm)")
		self.label_c_2.setToolTip('Distancia entre centros de llantas del lado izquierdo.')
		self.label_c_3 = self.findChild(QLabel,"label_cog_3")
		self.label_c_3.setText("𝒃<sub>f</sub> (mm)")
		self.label_c_3.setToolTip('Distancia entre llantas delanteras.')
		self.label_c_4 = self.findChild(QLabel,"label_cog_4")
		self.label_c_4.setText("𝒃<sub>t</sub> (mm)")
		self.label_c_4.setToolTip('Distancia entre llantas traseras.')
		self.label_c_5 = self.findChild(QLabel,"label_cog_5")
		self.label_c_5.setText("𝒅<sub>w der</sub> (mm)")
		self.label_c_5.setToolTip('Diámetro de llanta delantera derecha sin achatar.')
		self.label_c_6 = self.findChild(QLabel,"label_cog_6")
		self.label_c_6.setText("𝒅<sub>w izq</sub> (mm)")
		self.label_c_6.setToolTip('Diámetro de llanta delantera izquierda sin achatar.')
		self.label_c_7 = self.findChild(QLabel,"label_cog_7")
		self.label_c_7.setText("𝒅'<sub>w der</sub> (mm)")
		self.label_c_7.setToolTip('Diámetro de llanta delantera derecha achatada.')
		self.label_c_8 = self.findChild(QLabel,"label_cog_8")
		self.label_c_8.setText("𝒅'<sub>w izq</sub> (mm)")
		self.label_c_8.setToolTip('Diámetro de llanta delantera izquierda achatada.')
		self.label_c_9 = self.findChild(QLabel,"label_cog_9")
		self.label_c_9.setText("𝒉<sub>b</sub> (mm)")
		self.label_c_9.setToolTip('Altura de las básculas.')
		self.edit_c_1 = self.findChild(QLineEdit,"edit_cog_1")
		# self.line_c_1.setText("B<sub>1</sub> (kg)")
		self.edit_c_2 = self.findChild(QLineEdit,"edit_cog_2")
		# self.line_c_2.setText("B<sub>2</sub> (kg)")
		self.edit_c_3 = self.findChild(QLineEdit,"edit_cog_3")
		# self.line_c_3.setText("X<sub>cog</sub> (mm)")
		self.edit_c_4 = self.findChild(QLineEdit,"edit_cog_4")
		# self.line_c_4.setText("Y<sub>cog</sub> (mm)")
		self.edit_c_5 = self.findChild(QLineEdit,"edit_cog_5")
		# self.line_c_5.setText("Z<sub>cog</sub> (mm)")
		self.edit_c_6 = self.findChild(QLineEdit,"edit_cog_6")
		# self.line_c_3.setText("X<sub>cog</sub> (mm)")
		self.edit_c_7 = self.findChild(QLineEdit,"edit_cog_7")
		# self.line_c_4.setText("Y<sub>cog</sub> (mm)")
		self.edit_c_8 = self.findChild(QLineEdit,"edit_cog_8")
		# self.line_c_5.setText("Z<sub>cog</sub> (mm)")
		self.edit_c_9 = self.findChild(QLineEdit,"edit_cog_9")
		self.button_c_sig = self.findChild(QPushButton,"button_cog_sig")
		self.button_c_sig.clicked.connect(self.cog_sig_fase)
		self.button_c_sig.setEnabled(False)
		self.button_c_cap = self.findChild(QPushButton,"button_cog_cap")
		self.button_c_cap.clicked.connect(self.cog_capturar)
	...
	@pyqtSlot()
	def cog_capturar(self):
		print("Capturar datos CoG")
		if self.fase_cog == 1: 
			if self.checkNumber(self.edit_c_1.text()) and self.checkNumber(self.edit_c_2.text()) and self.checkNumber(self.edit_c_3.text()) and self.checkNumber(self.edit_c_4.text()) and self.checkNumber(self.edit_c_5.text()) and self.checkNumber(self.edit_c_6.text()) and self.checkNumber(self.edit_c_7.text()) and self.checkNumber(self.edit_c_8.text()) and self.checkNumber(self.edit_c_9.text()):
				self.param_cog['L_der'] = self.edit_c_1.text()	# Distancia llantas derechas
				self.param_cog['L_izq'] = self.edit_c_2.text()	# Distancia llantas izquierdas
				self.param_cog['bf'] = self.edit_c_3.text()		# Distancia llantas frontales
				self.param_cog['bt'] = self.edit_c_4.text()		# Distancia llantas traseras
				self.param_cog['dw_der'] = self.edit_c_5.text()	# Ancho llanta delantera derecha sin achatar
				self.param_cog['dw_izq'] = self.edit_c_6.text()	# Ancho llanta delantera izquierda sin achatar
				self.param_cog['d_w_der'] = self.edit_c_7.text()	# Ancho llanta delantera derecha achatada
				self.param_cog['d_w_izq'] = self.edit_c_8.text()	# Ancho llanta delantera derecha achatada
				self.param_cog['hb'] = self.edit_c_9.text()		# Altura de básculas
				self.button_c_cap.setEnabled(False)
				self.button_c_sig.setEnabled(True)
			else:
				box = QMessageBox()
				box.setIcon(QMessageBox.Question)
				box.setWindowTitle('Error')
				box.setText('Los datos deben ser numéricos.')
				box.setStandardButtons(QMessageBox.Ok)
				box.setWindowIcon(QIcon('icons/close_icon.png'))
				box.exec_()

				if box.clickedButton() == QMessageBox.Ok:
					print('Bye!')
					#self.stop_workers()
					box.done()
		
		if self.fase_cog == 2:
			self.param_cog['B1_f'].append(self.edit_c_1.text())		# Masa frontal - báscula 1 en plano
			self.param_cog['B2_f'].append(self.edit_c_2.text())	# Masa frontal - báscula 2 en plano
			if len(self.param_cog['B1_f'])>=3:
				self.button_c_cap.setEnabled(False)
				self.button_c_sig.setEnabled(True)

		if self.fase_cog == 3:
			self.param_cog['B1_t'].append(self.edit_c_1.text())		# Masa trasera - báscula 1 en plano
			self.param_cog['B2_t'].append(self.edit_c_2.text())		# Masa trasera - báscula 2 en plano
			if len(self.param_cog['B1_t'])>=3:
				self.button_c_cap.setEnabled(False)
				self.button_c_sig.setEnabled(True)
				
		if self.fase_cog == 4:
			if self.checkNumber(self.edit_c_3.text()) and self.checkNumber(self.edit_c_4.text()):
				self.param_cog['B1_th'].append(self.edit_c_1.text())	# Masa trasera - báscula 1 en elevación
				self.param_cog['B2_th'].append(self.edit_c_2.text())	# Masa trasera - báscula 2 en elevación
				self.param_cog['H1_f'].append(self.edit_c_3.text())		# Altura de elevación 1
				self.param_cog['H2_f'].append(self.edit_c_4.text())	# Altura de elevación 2
				if len(self.param_cog['B1_th'])>=3:
					self.button_c_cap.setEnabled(False)
					self.button_c_sig.setEnabled(True)
			else:
				box = QMessageBox()
	...
	@pyqtSlot()
	def cog_sig_fase(self):
		print("Siguiente fase CoG")
		self.fase_cog += 1
		if self.fase_cog>=5:
			self.fase_cog = 1
			print(self.param_cog)
			self.button_c_sig.setText('Siguiente')
		if self.fase_cog>=4:
			self.button_c_sig.setText('Finalizar')
			
		print(f'Fase {self.fase_cog} CoG: {self.fases_cog[self.fase_cog-1]}')
		self.label_c_tit2.setText(self.fases_cog[self.fase_cog-1])
		self.label_c_desc.setText(self.desc_cog[self.fase_cog-1])
		self.label_c_desc.setStyleSheet('QLabel { font-size:12pt; text-align: justify;}')

		self.button_c_cap.setEnabled(True)
		self.button_c_sig.setEnabled(False)

		if self.fase_cog==1:
			self.label_c_1.setText("𝑳<sub>der</sub> (mm)")
			self.label_c_1.setToolTip('Longitud entre centros de llantas del lado derecho.')
			self.label_c_1.show()
			self.edit_c_1.setReadOnly(False)
			self.edit_c_1.setStyleSheet('QLineEdit{	color: #000000; background-color: #ffffff; }')
			self.label_c_2.setText("𝑳<sub>izq</sub> (mm)")
			self.label_c_2.setToolTip('Longitud entre centros de llantas del lado izquierdo.')
			self.label_c_2.show()
			self.edit_c_2.setReadOnly(False)
			self.edit_c_2.setStyleSheet('QLineEdit{	color: #000000; background-color: #ffffff; }')
			self.label_c_3.setText("𝒃<sub>f</sub> (mm)")
			self.label_c_3.setToolTip('Longitud entre centros de llantas del lado derecho.')
			self.label_c_3.show()
			self.edit_c_3.show()
			self.label_c_4.setText("𝒃<sub>t</sub> (mm)")
			self.label_c_4.setToolTip('Longitud entre centros de llantas del lado derecho.')
			self.label_c_4.show()
			self.edit_c_4.show()
			self.label_c_5.setText("𝒅<sub>w der</sub> (mm)")
			self.label_c_5.setToolTip('Longitud entre centros de llantas del lado derecho.')
			self.label_c_5.show()
			self.edit_c_5.show()
			self.label_c_6.setText("𝒅<sub>w izq</sub> (mm)")
			self.label_c_6.setToolTip('Longitud entre centros de llantas del lado derecho.')
			self.label_c_6.show()
			self.edit_c_6.show()
			self.label_c_7.setText("𝒅'<sub>w der</sub> (mm)")
			self.label_c_7.setToolTip('Longitud entre centros de llantas del lado derecho.')
			self.label_c_7.show()
			self.edit_c_7.show()
			self.label_c_8.setText("𝒅'<sub>w izq</sub> (mm)")
			self.label_c_8.setToolTip('Longitud entre centros de llantas del lado derecho.')
			self.label_c_8.show()
			self.edit_c_8.show()
			self.label_c_9.show()
			self.edit_c_9.show()

		elif self.fase_cog in (2,3):
			self.label_c_1.setText("𝑩<sub>1</sub> (kg)")
			self.label_c_1.setToolTip('Masa en la báscula 1.')
			self.label_c_1.show()
			self.edit_c_1.setStyleSheet('QLineEdit{	color: #ffffff; background-color: transparent; border: white;}')
			self.edit_c_1.setReadOnly(True)
			self.label_c_2.setText("𝑩<sub>2</sub> (kg)")
			self.label_c_2.setToolTip('Masa en la báscula 2.')
			self.label_c_2.show()
			self.edit_c_2.setStyleSheet('QLineEdit{	color: #ffffff; background-color: transparent; border: white;}')
			self.edit_c_2.setReadOnly(True)
			self.label_c_3.hide()
			self.edit_c_3.hide()
			self.label_c_4.hide()
			self.edit_c_4.hide()
			...
			self.label_c_2.show()
			self.edit_c_2.setReadOnly(True)
			self.label_c_3.setText("𝑯<sub>der</sub> (mm)")
			self.label_c_3.setToolTip('Altura actual de la llanta delantera derecha.')
			self.label_c_3.show()
			self.edit_c_3.show()
			self.label_c_4.setText("𝑯<sub>izq</sub> (mm)")
			self.label_c_4.setToolTip('Altura actual de la llanta delantera izquierda.')
			self.label_c_4.show()
			self.edit_c_4.show()
			self.label_c_5.hide()
			self.edit_c_5.hide()
	...
	def update_plot(self):
		...
		# cdaq["B1"] = (round((cdaq["B1"])/0.015708))*0.015708/0.07854
		# cdaq["B2"] = (round((cdaq["B2"])/0.015708))*0.015708/0.07854
		# cdaq["B1"] = (cdaq["B1"])/0.003
		# cdaq["B2"] = (cdaq["B2"])/0.003
		...
		if self.tabs_pruebas.currentIndex()==0: #Prueba de frenado
			...
			if self.fase_frenado in (1,3,5):
				self.label_f_vx.setText(f'{round(vx,3)}')
				self.label_f_ax.setText(f'{round(gins["acc_y"],3)}')
				self.label_f_fp.setText(f'{round(cdaq["fp"],3)}')
				...
				if i==0:
					self.fre_widget_plot.setYRange(0,120)
				if i==1:
					self.fre_widget_plot.setYRange(-2*9.8,2*9.8)
				if i==2:
					self.fre_widget_plot.setYRange(0, 100*9.8)
		...
		if self.tabs_pruebas.currentIndex()==3: #Prueba de CoG
			if self.fase_cog>1:
				...
				# cdaq["B2"] *= sensB2
				# cdaq["B1"] = round((cdaq["B1"])/0.015708)*0.015708/0.07854
				# cdaq["B2"] = round((cdaq["B1"])/0.015708)*0.015708/0.07854
				self.edit_c_1.setText(f'{round(cdaq["B1"],2)}') # Báscula 1
				self.edit_c_2.setText(f'{round(cdaq["B2"],2)}') # Báscula 2
		...
		if self.tabs_pruebas.currentIndex()==4: #Modo de prueba
			...
			if self.combo_prueba.currentIndex()==0: #cDAQ-9188
				...
				self.label_p_v5r_velENav.setText(f'{round(cdaq["R5a"],3)}')
				...
				self.label_p_B1_yawGPS.setText(f'{round(cdaq["B1"],2)}')
				self.label_p_B2_velEGPS.setText(f'{round(cdaq["B2"],2)}')
...
class CDaq(QThread):
	...
	def list2dict(self):
		...
		eye_samps = np.ones(len(self.data_task1[0]))
		res["fp"] = 9.8*57.07*1e3*(self.data_task1[0,:])-eye_samps*cdaq_offset["fp"]
		#res["fp"] = 9.8*57.57*1e3*(self.data_task1[0,:])-eye_samps*cdaq_offset["fp"]
		#res["B1"] = round((self.data_task2[0,:])/0.015708)*0.015708/0.07854 - cdaq_offset["B1"] # mV
		#res["B2"] = round((self.data_task2[1,:])/0.015708)*0.015708/0.07854 - cdaq_offset["B2"] # mV
		res["B1"] = 2000*(self.data_task2[0,:])/0.07788 - eye_samps*cdaq_offset["B1"]	# mV
		res["B2"] = 2000*(self.data_task2[1,:])/0.07788 - eye_samps*cdaq_offset["B2"]	# mV
